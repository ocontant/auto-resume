...
def test_create_ats_prompt_handles_missing_sections(
    sample_resume_data_missing_sections,
):
...
def test_parse_llm_response_handles_missing_sections_in_response(
    llm_response_missing_sections, sample_resume_data
):
...
@pytest.mark.asyncio
async def test_optimize_resume_parsing_error(
    mock_litellm_acompletion, sample_resume_data, malformed_llm_response_content
):
